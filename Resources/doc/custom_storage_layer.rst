...
The user implementation must implement ``ZND\USM\UserBundle\Model\UserInterface``
and the user manager must implement ``ZND\USM\UserBundle\Model\UserManagerInterface``.
The ``ZND\USM\UserBundle\Model`` namespace provides base classes to make it easier to
